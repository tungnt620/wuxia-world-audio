...
const fromTextToAudio = (textContent, callbackOnError, callbackOnSuccess) => {
  if (!textContent) {
    ...
    rmrf(workingDir, () => {
      ...
      const chunkTextContent = chunkText(textContent, 4500);
      ...
      async.map(chunkTextContent, getTtsAudio, (err, audio) => {
        if (err) {
          ...
          async.eachOf(audio, writeAudioFiles, err => {
            if (err) {
              callbackOnError(
                "Failed to write audio segment(s) to disk.\n" + err
              );
            } else {
              fs.readdir(workingDir, (err, fileNames) => {
                fileNames.sort();
                let filePaths = fileNames.map(x => {
                  return path.join(workingDir, x);
                });
                concatAudioFiles(filePaths, (err, singleFilePath) => {
                  if (err) {
                    callbackOnError(
                      "Failed to concatinate audio files.\n" + err
                    );
                  } else {
