  ...
  return (
    <article className={'confession-detail'}>
      ...
      <div className={'relative-confessions'}>
        ...
        {
          relativeConfessions.map((confession, index) => (
            <TinyConfessionInfo key={confession.id} confession={confession} index={index}/>
          ))
        }
      </div>
        ...
        <h2>Bình luận</h2>
        {
          (comments || []).map(comment => {
            ...
            if (!comment.parent) {
              ...
              const commentChildren = (commentIDChildren[comment.id] || []).map(comment => <CommentBox
